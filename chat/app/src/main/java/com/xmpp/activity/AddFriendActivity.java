...
import com.xmpp.R;
import com.xmpp.adapter.AddFriendAdapter;
...
import com.xmpp.view.LoadingDialog;
...
public class AddFriendActivity extends Activity implements OnClickListener{
				...
				new Thread(new Runnable() {
					...
					public void run() {
						try {
							...
							XmppUtil.addUsers(roster,toUser+"@"+ MyApplication.xmppConnection.getServiceName(), toUser,"我的好友");
							...
							XmppUtil.sendMessage(MyApplication.xmppConnection, message,toUser);
	...
	private void searchUser() {
		...
		new Thread(new Runnable() {
			@Override
			public void run() {
				listUser=XmppUtil.searchUsers(MyApplication.xmppConnection, search_content);
				if(listUser.size()>0){
